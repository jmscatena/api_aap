...
def lista_usuarios(request):
  alunos = Usuarios.objects.all()
  return render(request, 'consultas/usuarios.html', {'alunos':alunos})

def lista_alunos(request):
  alunos = Usuarios.objects.filter(aluno__iexact='TRUE')
  return render(request, 'consultas/alunos.html', {'alunos':alunos})

def lista_professores(request):
  professores = Usuarios.objects.filter(professor__iexact='TRUE')
  return render(request, 'consultas/professores.html', {'professor':professores})

def lista_gestores(request):
  gestores = Usuarios.objects.filter(gestor__iexact='TRUE')
